...
def get_pokemon(g):
    ...
    for i in range(len(possible)):
        ...
        for j in range(len(possible[i])):
            if possible[i][j] == g[j]:
                score+=1
        scores.append([possible[i],score])

    return sorted(scores,reverse=True,key = lambda x:x[1])[0:10]
...
def helloworld(resp):
    ...
    if resp.event.message:
        ...
        if "The pokémon is" in content:
            ...
            print(get_pokemon(content.split(' ')[-1][:-1])[0])
            bot.sendMessage("895174767915720745", "p!c "+get_pokemon(content.split(' ')[-1][:-1])[0])
